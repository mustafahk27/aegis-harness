...
- Repo root: `/Users/mustafakhan/Documents/pi-harness`. All paths below are relative to it.
...
Run: `cd /Users/mustafakhan/Documents/pi-harness && npm i -D typescript vitest @earendil-works/pi-coding-agent typebox`
...
git commit -m "chore: scaffold pi-harness repo with vitest + typescript"
```
...
export function checkDangerous(command: string): string | null {
  if (/(^|\s|;|&&|\|\|)sudo\s/.test(` ${command} `)) {
    return "sudo is blocked by pi-harness. Run privileged commands manually.";
  }
...
describe("scanStagedDiff", () => {
  it("finds secrets in staged changes only", () => {
    const dir = mkdtempSync(join(tmpdir(), "ph-secrets-"));
...
function tmp(): string {
  return mkdtempSync(join(tmpdir(), "ph-stack-"));
}
...
const cwd = () => mkdtempSync(join(tmpdir(), "ph-checks-"));
...
export function personaPrompt(): string {
  return `
## Engineering discipline (pi-harness)
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (missing.length && ctx.hasUI) {
      ctx.ui.notify(
        `pi-harness: ${missing.join(", ")} not installed — falling back to built-in scanning only`,
        "warning",
      );
    }
    if (ctx.hasUI) ctx.ui.setStatus("pi-harness", "gates: on");
  });
    ...
    if (isToolCallEventType("bash", event)) {
      ...
      if (gatesEnabled && isGitCommit(command)) {
        ...
        try {
          ...
          return { block: true, reason: `pi-harness secret scan failed (fail-closed): ${String(err)}` };
        }
        if (findings.length) return { block: true, reason: formatSecrets(findings) };

        // Commit gate: full check suite must pass.
        if (ctx.hasUI) ctx.ui.setStatus("pi-harness", "running pre-commit checks…");
        const results = runChecks(ctx.cwd, detectStack(ctx.cwd).checks);
        if (ctx.hasUI) ctx.ui.setStatus("pi-harness", "gates: on");
        const failed = results.filter((r) => !r.ok);
        if (failed.length) {
          return { block: true, reason: `Commit blocked by pi-harness.\n${formatFailures(results)}` };
        }
  ...
  pi.on("agent_end", async () => {
    if (gatesEnabled && doneGate.shouldBounce()) {
      pi.sendMessage(
        {
          customType: "pi-harness-done-gate",
          content:
            "pi-harness done gate: you modified code this session but there was no passing test run afterwards. " +
  ...
  pi.registerCommand("check", {
    description: "pi-harness: run the full check suite for this project",
  ...
  pi.registerCommand("secreview", {
    description: "pi-harness: security-review the current uncommitted diff",
    handler: async (_args, ctx) => {
  ...
  pi.registerCommand("gates", {
    description: "pi-harness: on|off|status — toggle commit/secret/done gates (dangerous-command gate always on)",
    handler: async (args, ctx) => {
      const arg = (args ?? "").trim();
      if (arg === "on") gatesEnabled = true;
      else if (arg === "off") gatesEnabled = false;
      ctx.ui.setStatus("pi-harness", `gates: ${gatesEnabled ? "on" : "OFF"}`);
      ctx.ui.notify(
        `pi-harness gates ${gatesEnabled ? "ON" : "OFF — commit/secret/done gates disabled until /gates on or session restart"}`,
...
```bash
mkdir -p /tmp/ph-smoke && cd /tmp/ph-smoke && git init -q
pi -e /Users/mustafakhan/Documents/pi-harness/extension/index.ts
```

Then in the pi session:
1. Prompt: `run the command: sudo ls` → expect the tool call to be blocked with the pi-harness sudo reason.
...
- A change is not done until the full suite passes. The pi-harness done gate will
...
- **Never commit:** secrets or credentials (the pi-harness secret gate blocks these),
...
Run: `pi --skill /Users/mustafakhan/Documents/pi-harness/skills/tdd -p "List the names and descriptions of the skills you have available, then stop."`
...
ln -sfn "${REPO_DIR}/extension" "${PI_DIR}/extensions/pi-harness"
ln -sfn "${REPO_DIR}/skills" "${PI_DIR}/skills/pi-harness"

echo "pi-harness installed:"
echo "  ${PI_DIR}/extensions/pi-harness -> ${REPO_DIR}/extension"
echo "  ${PI_DIR}/skills/pi-harness     -> ${REPO_DIR}/skills"
...
```bash
mkdir -p /tmp/ph-e2e && cd /tmp/ph-e2e && git init -q
...
Run: `cd /Users/mustafakhan/Documents/pi-harness && npx vitest run && npx tsc --noEmit`
Expected: PASS, clean
